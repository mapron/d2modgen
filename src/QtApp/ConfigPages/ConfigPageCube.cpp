    ...
    : ConfigPageAbstract(parent)
{
    ...
    addEditors(QList<IValueWidget*>()
               << makeEditor("noGemUpgrade", tr("Remove gem component from Rune upgrade recipes (Rune x3,x2 => next Rune)"))
               << makeEditor("quickPortals", tr("Add quick portal access recipes:") + "\n"
                                                                                      "1. TP book + Id scroll = Cow Portal\n"
                                                                                      "2. TP book + Id scroll x2 = Minor Uber Portal\n"
                                                                                      "3. TP book + Id scroll x3 = Uber Tristram Portal")
               << makeEditor("quickQuests", tr("Add quick quest recipes:") + " "
                                                                             "1. key + Id scroll = Horadric Staff "
                                                                             "2. key + TP scroll = Khalim Will")
               << makeEditor("socketing", tr("Add socketing recipes:") + "\n"
               ...
               << makeEditor("upgrading", tr("Add upgrade recipes:") + "\n"
               ...
               << makeEditor("runeDowngrade", tr("Add Rune downgrade:") + " Rune + key + key = Lower rune"));
    addWidget(new QLabel(tr("Simplyfy crafting:"), this));
    addEditors(QList<IValueWidget*>()
               << makeEditor("craftNoStrict", tr("Remove strict item types for Crafted (any helm, any gloves etc)"))
               << makeEditor("craftNoRunes", tr("Remove Rune/Jewel requirement for Crafted\n"
                                                "(So any recipe is 'item + gem')"))
               << makeEditor("craftHighIlvl", tr("Make Item Level always 99"))
               << makeEditor("craftMultBonus", tr("Multiply crafted item bonuses by")));
    closeLayout();
}
