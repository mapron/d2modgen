...
Item {
    ...
    Connections {
        ...
        function onDataChanged() {
            if (root.value_context) {
                let newValue = root.value_context.getStr(root.value_key);
    ...
    onDynamicValueChanged: {
        if (root.value_context) {
            ...
            let currentRemoteValue = root.value_context.getStr(root.value_key);
    ...
    onValue_contextChanged: {
        if (root.value_context) {
            root.dynamicValue = root.value_context.getStr(root.value_key);
        }
    }
}
